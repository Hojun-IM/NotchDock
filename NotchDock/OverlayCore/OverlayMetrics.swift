...
enum OverlayMetrics {
    ...
    enum Header {
        ...
        /// 오버레이 상단에서 헤더 줄까지 비워 둘 높이. 노치/독이 같은 값을 쓴다.
        ///
        /// 노치 모드에서도 물리 노치(카메라 하우징)는 화면 "가운데"에만 있고 헤더 항목은
        /// 좌우 끝에 있으므로, 콘텐츠 전체를 노치 높이(약 30~38pt)만큼 내릴 필요가 없다.
        /// 그렇게 내리면 헤더만 아래로 밀려 두 모드의 헤더 위치가 어긋나고 본문도 좁아진다.
